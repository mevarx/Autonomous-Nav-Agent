...
from src.environment import SnakeGameAI, Direction, Point
...
class Agent:
    ...
    def train_long_memory(self):
        if len(self.memory) > config.BATCH_SIZE:
            mini_sample = random.sample(self.memory, config.BATCH_SIZE)
        else:
...
def train():
    ...
    while True:
        ...
        if done:
            # Train long memory (replay memory) and plot
            game.reset()
            agent.n_games += 1
